...
def generate_launch_description():
    ...
    # Expand nested model includes before publishing the description. Gazebo
    # understands model:// mesh URIs, while RViz's resource loader does not.
    # Convert only this RViz / robot_state_publisher copy to file URIs; the SDF
    # loaded by Gazebo remains unchanged.
    robot_desc = subprocess.check_output(
        ["gz", "sdf", "-p", sdf_file], text=True
    )
    models_uri = f"file://{os.path.join(pkg_ardupilot_gazebo, 'models')}/"
    robot_desc = robot_desc.replace("model://", models_uri)
